...
-- Sample report cards
INSERT INTO report_cards (id, student_id, evaluation_period_id, generated_by, generation_date, overall_average, class_rank, total_students, teacher_comments, file_path) VALUES
(1, 1, 1, 3, NOW(), 80.00, 8, 25, 'Good consistent performance across subjects.', '/reports/s2025001_seq1.pdf'),
(2, 2, 1, 3, NOW(), 90.75, 2, 25, 'Excellent academic performance. Keep it up!', '/reports/s2025002_seq1.pdf')
ON CONFLICT (id) DO UPDATE SET 
...
(1, 1, 'CREATE', 'students', 1, NULL, '{"student_number": "S2025001", "classroom_id": 1}', '192.168.1.1', 'Mozilla/5.0', NOW()),
(2, 1, 'UPDATE', 'grades', 1, '{"points_earned": 15}', '{"points_earned": 16}', '192.168.1.1', 'Mozilla/5.0', NOW())
ON CONFLICT (id) DO UPDATE SET 
    action = EXCLUDED.action,
    created_at = EXCLUDED.created_at;

-- Update sequences to prevent primary key conflicts
SELECT setval('users_id_seq', GREATEST((SELECT MAX(id) FROM users), 25));
SELECT setval('teachers_id_seq', GREATEST((SELECT MAX(id) FROM teachers), 10));
SELECT setval('classrooms_id_seq', GREATEST((SELECT MAX(id) FROM classrooms), 9));
SELECT setval('subjects_id_seq', GREATEST((SELECT MAX(id) FROM subjects), 24));
SELECT setval('teacher_subject_classroom_id_seq', GREATEST((SELECT MAX(id) FROM teacher_subject_classroom), 22));
SELECT setval('evaluation_periods_id_seq', GREATEST((SELECT MAX(id) FROM evaluation_periods), 8));
SELECT setval('evaluation_types_id_seq', GREATEST((SELECT MAX(id) FROM evaluation_types), 5));
SELECT setval('evaluations_id_seq', GREATEST((SELECT MAX(id) FROM evaluations), 4));
SELECT setval('students_id_seq', GREATEST((SELECT MAX(id) FROM students), 14));
SELECT setval('grades_id_seq', GREATEST((SELECT MAX(id) FROM grades), 8));
SELECT setval('attendances_id_seq', GREATEST((SELECT MAX(id) FROM attendances), 4));
SELECT setval('report_cards_id_seq', GREATEST((SELECT MAX(id) FROM report_cards), 2));
SELECT setval('audit_logs_id_seq', GREATEST((SELECT MAX(id) FROM audit_logs), 2));